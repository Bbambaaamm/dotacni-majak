    ...
    with tempfile.TemporaryDirectory() as tmp:
        async with GuardedHttpClient(
            ...
            if health.status.value != "HEALTHY":
                raise SystemExit(f"Pardubický source is not healthy: {health.detail}")
            if not page.items:
                raise SystemExit("Pardubický live smoke discovered zero programmes")

            sample = page.items[0]
            record = (await adapter.fetch_record(ctx, sample)).record
            if record is None or not record.snapshot_ids:
                raise SystemExit("Pardubický detail did not produce RAW-backed record")
