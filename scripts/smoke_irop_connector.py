...
async def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        async with GuardedHttpClient(
            ...
            if not page.items:
                raise SystemExit("IROP live smoke discovered zero calls")
            record = (await adapter.fetch_record(ctx, page.items[0])).record
            if record is None:
                raise SystemExit("IROP detail returned no record")
            print(
                "sample",
                record.external_id,
                record.native_status,
                record.raw_fields.get("allocationCzk"),
            )
